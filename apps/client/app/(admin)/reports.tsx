...
import { IndianRupee, Users, Calendar, Download, Search, ChevronRight, CheckCircle2, Clock } from 'lucide-react-native';
  ...
  return (
    <View style={{ backgroundColor: '#f8fafc' }} className="flex-1 p-6">
      ...
      <View className="flex-row bg-slate-100 p-1.5 rounded-2xl mb-6">
        ...
        >
          <Text className={`font-bold text-sm ${activeTab === 'meetings' ? 'text-slate-800' : 'text-slate-500'}`}>Meeting Performance</Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => setActiveTab('members')}
          className={`flex-1 py-3 rounded-xl items-center ${activeTab === 'members' ? 'bg-white shadow-sm' : 'bg-transparent'}`}
        >
          <Text className={`font-bold text-sm ${activeTab === 'members' ? 'text-slate-800' : 'text-slate-500'}`}>Member Attendance History</Text>
        </TouchableOpacity>
      </View>
                ...
                renderItem={({ item }) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      className={`p-3.5 mb-2 rounded-xl flex-row items-center justify-between border transition-all duration-150 ${
                        isSelected 
